...
export class SoundSystem {
  ...
  playQuestComplete() {
      ...
      // Crisp Digital "Ding" - High pitch, short decay
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
  ...
  playBossWarning() {
      ...
      // Deep throbbing alarm
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      
      osc.type = 'sawtooth';
      osc.frequency.setValueAtTime(50, t);
      osc.frequency.linearRampToValueAtTime(40, t + 1.0); // Pitch drop
      
      // Pulse effect via gain modulation
