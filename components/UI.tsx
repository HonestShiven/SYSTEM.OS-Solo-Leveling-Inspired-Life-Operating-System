...
export const Panel = ({ children, className = '', title, accentColor = 'blue' }: { children?: React.ReactNode, className?: string, title?: string, accentColor?: 'blue' | 'red' | 'green' | 'purple' | 'yellow' }) => {
  ...
  return (
    <div className={`relative flex flex-col ${className}`}>
        ...
        <div className={`relative flex-1 bg-system-panel border ${borderColor} border-opacity-30 backdrop-blur-sm overflow-hidden min-h-0`}>
             {/* Tech Corners */}
             <div className={`absolute top-0 left-0 w-2 h-2 border-l border-t ${borderColor} z-20`}></div>
             <div className={`absolute top-0 right-0 w-2 h-2 border-r border-t ${borderColor} z-20`}></div>
             <div className={`absolute bottom-0 left-0 w-2 h-2 border-l border-b ${borderColor} z-20`}></div>
             <div className={`absolute bottom-0 right-0 w-2 h-2 border-r border-b ${borderColor} z-20`}></div>
            ...
            {/* Absolute inset wrapper to force correct height for scrolling */}
            <div className={`absolute inset-0 z-10 flex flex-col ${title ? 'p-0' : 'p-4'}`}>
                {children}
            </div>
        </div>
    </div>
